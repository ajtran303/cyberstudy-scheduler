...
async function main() {
  ...
  const ethHack = await prisma.course.create({
    data: {
      ...
      topics: {
        create: [
          { name: "Reconnaissance & OSINT", date: new Date("2026-03-05"), mastery: "HARDENED", lastReviewedAt: new Date("2026-03-02"), sortOrder: 0, keyTerms: [{ term: "OSINT", definition: "Open-Source Intelligence — gathering information from publicly available sources for security analysis" }, { term: "Shodan", definition: "Search engine that indexes internet-connected devices, revealing exposed services, ports, and vulnerabilities" }, { term: "Google Dorking", definition: "Using advanced Google search operators (site:, filetype:, inurl:) to find sensitive information exposed on websites" }, { term: "WHOIS", definition: "Protocol for querying domain registration databases to find ownership, contact, and nameserver information" }] },
          { name: "Scanning & Enumeration", date: new Date("2026-03-12"), mastery: "SCANNING", lastReviewedAt: new Date("2026-03-01"), sortOrder: 1, details: "Nmap, Masscan, service detection", keyTerms: [{ term: "Nmap", definition: "Network Mapper — open-source tool for network discovery and security auditing that scans hosts for open ports and services" }, { term: "SYN Scan", definition: "Half-open TCP scan that sends SYN packets without completing the handshake, making it faster and stealthier than a full connect scan" }, { term: "Banner Grabbing", definition: "Technique of connecting to a service and reading its response banner to identify software name, version, and OS" }] },
          { name: "Exploitation Basics", date: new Date("2026-03-19"), mastery: "EXPOSED", sortOrder: 2, keyTerms: [{ term: "CVE", definition: "Common Vulnerabilities and Exposures — standardized identifier system for publicly known security vulnerabilities" }, { term: "Payload", definition: "The code or commands delivered by an exploit that perform the attacker's intended action on the target system" }, { term: "Metasploit", definition: "Open-source penetration testing framework providing exploit modules, payloads, and post-exploitation tools" }] },
  ...
  await prisma.course.create({
    data: {
      ...
      topics: {
        create: [
          { name: "Symmetric Encryption", mastery: "EXPOSED", sortOrder: 0, keyTerms: [{ term: "AES", definition: "Advanced Encryption Standard — symmetric block cipher using 128/192/256-bit keys, the most widely used encryption algorithm" }, { term: "Block Cipher", definition: "Encryption algorithm that processes data in fixed-size blocks (e.g. 128 bits) using a symmetric key" }, { term: "Key Schedule", definition: "Algorithm that expands a cipher key into multiple round keys used in each round of encryption" }] },
          { name: "Asymmetric Encryption", mastery: "EXPOSED", sortOrder: 1, keyTerms: [{ term: "RSA", definition: "Rivest-Shamir-Adleman — public-key cryptosystem based on the difficulty of factoring large prime numbers" }, { term: "Public Key", definition: "The freely shared half of an asymmetric key pair, used to encrypt data or verify digital signatures" }, { term: "Key Exchange", definition: "Protocol (e.g. Diffie-Hellman) allowing two parties to establish a shared secret over an insecure channel" }] },
          { name: "Hash Functions", mastery: "EXPOSED", sortOrder: 2, keyTerms: [{ term: "SHA-256", definition: "Secure Hash Algorithm producing a 256-bit digest, widely used for data integrity verification and digital signatures" }, { term: "Collision Resistance", definition: "Property of a hash function where it is computationally infeasible to find two different inputs producing the same hash output" }, { term: "Salt", definition: "Random data added to a password before hashing to prevent rainbow table attacks and ensure identical passwords produce different hashes" }] },
